...
def test_blocked_3prime_veto():
    ...
    # Utilisons un exemple qui est amplifiable: A a une matrice sur B.
    seq_a = "ATGCATGCGCG"
    seq_b = "TTTTCGCGCATGCAT" 
    ...
    p_a_blocked = replace(p_a, blocked_3prime=True)
    p_b_blocked = replace(p_b, blocked_3prime=True)
    is_amp, dg_3p, details = evaluate_pair_amplifiable(p_a_blocked, p_b_blocked, backend, BST, 65.0)
    assert is_amp == False
...
def test_probe_tm_check():
    ...
    probe_seq = "GCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGCGC"
    ...
    assert res[0]["is_ok"] == True # Probe is very GC rich and long, Tm should be much higher than AT-rich primers
